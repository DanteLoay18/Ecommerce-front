...
export default function MainOfertas({products, categories}){
    ...
    function obtenerProductosConOfertas(){
        let arrPro=[];
        products.map(producto =>{
            
            arrPro.push(tieneOfertas(producto));
        })
        setProductos(arrPro)
      }
    ...
    return (
        <div>
            ...
            <h2 className="title">Lista de productos con ofertas</h2>
    
            <div className="product-grid">
            {productos && (
              <h5>No hay ofertas en este momento</h5>
            )}
            {productos.length >0 && productos.filter((product) => tieneOfertas(product)) // Filter products based on tieneFavoritos function
                                             .map(product => (
